...
- The newest refinement round is `eval_043_eval038_ttt_epochs_pair_clean_idle_blocked`, which stopped before launch because the clean-idle gate on pinned GPUs `0..7` never passed.
...
- Clean-idle gate result on GPUs `0,1,2,3,4,5,6,7`:
  ...
  - compute-app snapshots stayed `PID 3676150 / [Not Found] / 74486 MiB`
...
- Completed the reviewed exact promoted-line same-session epoch-pair attempt as a controlled blocker report.
- Controlled intervention actually executed:
  ...
  - on-disk recording of intended control/candidate commands and gate evidence in `runs/eval_043_eval038_ttt_epochs_pair_clean_idle_blocked/`
