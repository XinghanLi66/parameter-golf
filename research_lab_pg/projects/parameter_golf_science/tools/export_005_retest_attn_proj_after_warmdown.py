...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Controlled warmdown-checkpoint export retest with uniform int8, MLP-only int6, and MLP+attn.proj int6."
    )
    ...
    parser.add_argument("--historical-summary", required=True, help="Path to export_004 summary.json for historical comparison.")
    ...
    historical_summary_path = Path(cli.historical_summary).resolve()
    ...
    log0("experiment_id:export_005_retest_attn_proj_subset_after_warmdown_gain", rank)
    log0(f"checkpoint:{checkpoint_path}", rank)
    log0(f"historical_summary:{historical_summary_path}", rank)
    ...
    results: dict[str, Any] = {
        "experiment_id": "export_005_retest_attn_proj_subset_after_warmdown_gain",
    ...
    for policy_name, default_bits in policies:
        ...
        log0(
            f"artifact:{policy_name} default_bits:{default_bits} label:{compression_label} bytes:{artifact_bytes} total:{total_submission_bytes} "
    ...
    if rank == 0:
        baseline = results["artifacts"]["uniform_int8"]
        prior_safe = results["artifacts"]["mlp_int6_else_int8"]
        candidate = results["artifacts"]["mlp_int6_plus_attn_proj_int6_else_int8"]
        historical_candidate = historical_summary["artifacts"]["mlp_int6_plus_attn_proj_int6_else_int8"]
        results["comparison"] = {
            "vs_uniform_int8": {
                "artifact_bytes_delta": candidate["artifact_bytes"] - baseline["artifact_bytes"],
                "artifact_bytes_delta_percent": 100.0 * (candidate["artifact_bytes"] - baseline["artifact_bytes"]) / baseline["artifact_bytes"],
                "total_submission_bytes_delta": candidate["total_submission_bytes"] - baseline["total_submission_bytes"],
                "payload_bytes_delta": candidate["payload_bytes"] - baseline["payload_bytes"],
                "payload_bytes_delta_percent": 100.0 * (candidate["payload_bytes"] - baseline["payload_bytes"]) / baseline["payload_bytes"],
                "raw_torch_bytes_delta": candidate["raw_torch_bytes"] - baseline["raw_torch_bytes"],
                "val_bpb_delta": candidate["val_bpb"] - baseline["val_bpb"],
                "quantization_gap_bpb_delta": candidate["quantization_gap_bpb"] - baseline["quantization_gap_bpb"],
                "export_time_ms_delta": candidate["export_time_ms"] - baseline["export_time_ms"],
                "load_time_ms_delta": candidate["load_time_ms"] - baseline["load_time_ms"],
            },
            "vs_prior_safe_mlp_int6": {
                "artifact_bytes_delta": candidate["artifact_bytes"] - prior_safe["artifact_bytes"],
                "artifact_bytes_delta_percent": 100.0 * (candidate["artifact_bytes"] - prior_safe["artifact_bytes"]) / prior_safe["artifact_bytes"],
                "total_submission_bytes_delta": candidate["total_submission_bytes"] - prior_safe["total_submission_bytes"],
                "payload_bytes_delta": candidate["payload_bytes"] - prior_safe["payload_bytes"],
                "payload_bytes_delta_percent": 100.0 * (candidate["payload_bytes"] - prior_safe["payload_bytes"]) / prior_safe["payload_bytes"],
                "raw_torch_bytes_delta": candidate["raw_torch_bytes"] - prior_safe["raw_torch_bytes"],
                "val_bpb_delta": candidate["val_bpb"] - prior_safe["val_bpb"],
                "quantization_gap_bpb_delta": candidate["quantization_gap_bpb"] - prior_safe["quantization_gap_bpb"],
                "export_time_ms_delta": candidate["export_time_ms"] - prior_safe["export_time_ms"],
                "load_time_ms_delta": candidate["load_time_ms"] - prior_safe["load_time_ms"],
            },
            "vs_historical_export_004_candidate": {
                "artifact_bytes_delta": candidate["artifact_bytes"] - historical_candidate["artifact_bytes"],
                "artifact_bytes_delta_percent": 100.0 * (candidate["artifact_bytes"] - historical_candidate["artifact_bytes"]) / historical_candidate["artifact_bytes"],
                "total_submission_bytes_delta": candidate["total_submission_bytes"] - historical_candidate["total_submission_bytes"],
                "payload_bytes_delta": candidate["payload_bytes"] - historical_candidate["payload_bytes"],
                "payload_bytes_delta_percent": 100.0 * (candidate["payload_bytes"] - historical_candidate["payload_bytes"]) / historical_candidate["payload_bytes"],
                "raw_torch_bytes_delta": candidate["raw_torch_bytes"] - historical_candidate["raw_torch_bytes"],
                "val_bpb_delta": candidate["val_bpb"] - historical_candidate["val_bpb"],
                "quantization_gap_bpb_delta": candidate["quantization_gap_bpb"] - historical_candidate["quantization_gap_bpb"],
                "export_time_ms_delta": candidate["export_time_ms"] - historical_candidate["export_time_ms"],
                "load_time_ms_delta": candidate["load_time_ms"] - historical_candidate["load_time_ms"],
            },
        }
