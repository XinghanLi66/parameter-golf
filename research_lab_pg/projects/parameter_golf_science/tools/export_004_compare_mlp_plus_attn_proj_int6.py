...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Controlled uniform int8 vs MLP+attn.proj int6 export comparison on a locked checkpoint."
    )
    ...
    parser.add_argument("--prior-summary", required=True, help="Path to export_003 summary.json for prior frontier comparison.")
    ...
    prior_summary_path = Path(cli.prior_summary).resolve()
    ...
    log0("experiment_id:export_004_expand_selective_int6_beyond_safe_mlp_subset", rank)
    log0(f"checkpoint:{checkpoint_path}", rank)
    log0(f"prior_summary:{prior_summary_path}", rank)
    log0(f"eval_config:mode={args.eval_mode} stride={args.eval_stride}", rank)
    log0(f"compression_config:zstd-{cli.zstd_level}", rank)
    log0("precision_config:uniform_int8 vs mlp_int6_plus_attn_proj_int6_else_int8", rank)
    ...
    results: dict[str, Any] = {
        "experiment_id": "export_004_expand_selective_int6_beyond_safe_mlp_subset",
        "checkpoint": str(checkpoint_path),
        "train_script": str(train_script_path),
        "prior_summary": str(prior_summary_path),
    ...
    if rank == 0:
        ...
        prior_candidate = prior_summary["artifacts"]["mlp_int6_else_int8"]
        results["comparison"] = {
            "vs_uniform_int8": {
            ...
            "vs_prior_mlp_int6": {
                "artifact_bytes_delta": candidate["artifact_bytes"] - prior_candidate["artifact_bytes"],
                "artifact_bytes_delta_percent": 100.0 * (candidate["artifact_bytes"] - prior_candidate["artifact_bytes"]) / prior_candidate["artifact_bytes"],
                "total_submission_bytes_delta": candidate["total_submission_bytes"] - prior_candidate["total_submission_bytes"],
                "payload_bytes_delta": candidate["payload_bytes"] - prior_candidate["payload_bytes"],
                "payload_bytes_delta_percent": 100.0 * (candidate["payload_bytes"] - prior_candidate["payload_bytes"]) / prior_candidate["payload_bytes"],
                "raw_torch_bytes_delta": candidate["raw_torch_bytes"] - prior_candidate["raw_torch_bytes"],
                "val_bpb_delta": candidate["val_bpb"] - prior_candidate["val_bpb"],
                "quantization_gap_bpb_delta": candidate["quantization_gap_bpb"] - prior_candidate["quantization_gap_bpb"],
                "export_time_ms_delta": candidate["export_time_ms"] - prior_candidate["export_time_ms"],
                "load_time_ms_delta": candidate["load_time_ms"] - prior_candidate["load_time_ms"],
            },
        }
