...
def main() -> int:
    parser = argparse.ArgumentParser(description="Controlled zlib vs zstd export comparison on a locked checkpoint.")
    ...
    log0(f"experiment_id:export_001_zstd_only_after_eval_lockin", rank)
    log0(f"checkpoint:{checkpoint_path}", rank)
    log0(f"eval_config:mode={args.eval_mode} stride={args.eval_stride}", rank)
    log0(f"compression_config:zlib-{cli.zlib_level} vs zstd-{cli.zstd_level}", rank)
    ...
    quant_obj, quant_stats = root.quantize_state_dict_int8(checkpoint_state)
    ...
    results: dict[str, Any] = {
        "experiment_id": "export_001_zstd_only_after_eval_lockin",
    ...
    for backend, level in backends:
        export_start = time.perf_counter()
        blob, compression_label = root.compress_export_bytes(quant_raw, backend=backend, level=level)
        export_time_ms = 1000.0 * (time.perf_counter() - export_start)

        artifact_path = output_dir / f"final_model.int8.{backend}.ptz"
        if rank == 0:
        ...
        decompressed = root.decompress_export_bytes(blob_disk, backend=backend)
        ...
        base_model.load_state_dict(checkpoint_state, strict=True)
        base_model.load_state_dict(root.dequantize_state_dict_int8(quant_state), strict=True)
        artifact_eval = root.evaluate_modes(
            args,
            model,
            rank,
            world_size,
        ...
        val_loss, val_bpb, eval_time_ms = artifact_eval[args.eval_mode]
        artifact_bytes = artifact_path.stat().st_size
        total_submission_bytes = artifact_bytes + code_bytes

        results["artifacts"][backend] = {
        ...
        log0(
            f"artifact:{backend} label:{compression_label} bytes:{artifact_bytes} total:{total_submission_bytes} "
            f"export_time:{export_time_ms:.2f}ms load_time:{load_time_ms:.2f}ms "
            f"roundtrip_exact:{roundtrip_exact} val_bpb:{val_bpb:.8f} gap:{val_bpb - checkpoint_val_bpb:+.8f} "
            f"eval_time:{eval_time_ms:.0f}ms",
            rank,
        )

    if rank == 0:
        zlib_result = results["artifacts"]["zlib"]
        zstd_result = results["artifacts"]["zstd"]
        results["comparison"] = {
            "artifact_bytes_delta": zstd_result["artifact_bytes"] - zlib_result["artifact_bytes"],
            "artifact_bytes_delta_percent": (
                100.0 * (zstd_result["artifact_bytes"] - zlib_result["artifact_bytes"]) / zlib_result["artifact_bytes"]
            ),
            "total_submission_bytes_delta": zstd_result["total_submission_bytes"] - zlib_result["total_submission_bytes"],
            "val_bpb_delta": zstd_result["val_bpb"] - zlib_result["val_bpb"],
            "export_time_ms_delta": zstd_result["export_time_ms"] - zlib_result["export_time_ms"],
            "load_time_ms_delta": zstd_result["load_time_ms"] - zlib_result["load_time_ms"],
        }
