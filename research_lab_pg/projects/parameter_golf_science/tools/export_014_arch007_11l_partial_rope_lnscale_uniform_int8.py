...
def build_eval_args(root: ModuleType, cli: argparse.Namespace) -> Any:
    ...
    return SimpleNamespace(
        ...
        rope_dims=cli.rope_dims if cli.rope_dims is not None else defaults.rope_dims,
        ...
        xsa_last_n=cli.xsa_last_n if cli.xsa_last_n is not None else defaults.xsa_last_n,
        ln_scale=bool(cli.ln_scale) if cli.ln_scale is not None else defaults.ln_scale,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Controlled uniform-int8 export comparison for arch_007 Partial RoPE + LN Scale.")
    parser.add_argument("--experiment-id", required=True, help="Experiment identifier for the summary JSON.")
    ...
    parser.add_argument("--baseline-summary", required=True, help="Path to the locked arch_005 summary.json baseline.")
    ...
    parser.add_argument("--ln-scale", type=int, choices=[0, 1], default=None)
    ...
    baseline_experiment_id = baseline_summary.get("experiment_id", "baseline")
    comparison_key = f"vs_locked_{baseline_experiment_id}_uniform_int8"

    log0(f"experiment_id:{cli.experiment_id}", rank)
    ...
    log0("precision_config:uniform_int8", rank)
    ...
    quant_obj, quant_stats = root.quantize_state_dict_mixed_precision(checkpoint_state, default_num_bits=8, bit_overrides={})
    ...
    artifact_path = output_dir / "final_model.uniform_int8.zstd.ptz"
    if rank == 0:
    ...
    log0(
        f"artifact:uniform_int8 bytes:{artifact_bytes} total:{total_submission_bytes} "
    ...
    if rank == 0:
        results = {
            ...
            "artifacts": {"uniform_int8": candidate_result},
            "comparison": {
                comparison_key: {
                    "artifact_bytes_delta": artifact_bytes - baseline_uniform["artifact_bytes"],
                    "total_submission_bytes_delta": total_submission_bytes - baseline_uniform["total_submission_bytes"],
                    "val_bpb_delta": val_bpb - baseline_uniform["val_bpb"],
                    "quantization_gap_bpb_delta": quantization_gap_bpb - baseline_uniform["quantization_gap_bpb"],
                    "checkpoint_val_bpb_delta": checkpoint_val_bpb - baseline_summary["checkpoint_eval"]["val_bpb"],
                }
            },
        }
